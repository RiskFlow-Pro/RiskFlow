...
async function fetchHLTicker(symbol) {
  ...
  try {
    ...
    const mid = data?.[coin]; if(!mid) return null;
...
async function fetchHLContractInfo(symbol) {
  ...
  try {
    ...
    const szDec = asset.szDecimals??3, step = Math.pow(10,-szDec);
    return {pricePlace:asset.pxDecimals??2,sizeMultiplier:step,minTradeNum:step};
...
async function fetchContractInfo(symbol){
  ...
  const isWeex  = window._activeExchange === 'weex';
  const isBybit = window._activeExchange === 'bybit' || window._activeExchange === 'bybit_demo';
  if (window._activeExchange === 'hyperliquid') { const info=await fetchHLContractInfo(symbol); _contractInfoCache[symbol]=info; return info; }
  const isHL    = window._activeExchange === 'hyperliquid';
  if (isHL) { const info = await fetchHLContractInfo(symbol); _contractInfoCache[symbol]=info; return info; }
  if (isBingx) {
...
async function executeOrder(){
  ...
  // ─── HYPERLIQUID ORDER (early return — non tocca la catena if/else esistente) ───
  if (isHL) {
    ...
    try {
      ...
      console.error('[HL ORDER]',e);
    }
    return; // early return — non esegue il resto
  }
      ...
      if (isHL) {
        ...
        positions = (balData?.assetPositions||[])
          ...
          .map(ap=>{
            ...
            return {symbol:p.coin+'USDT',holdSide:side,total:Math.abs(szi),available:Math.abs(szi),
              openPriceAvg:parseFloat(p.entryPx||0),markPrice:parseFloat(p.markPx||0),
              unrealizedPL:parseFloat(p.unrealizedPnl||0),leverage:parseFloat(p.leverage?.value||1),
              liquidationPrice:parseFloat(p.liquidationPx||0),stopLoss:0,takeProfit:0,
              marginMode:p.leverage?.type==='isolated'?'isolated':'crossed',_hl:true,_hlCoin:p.coin};
          });
      } else
      if (isBingx) {
        const bsym = toBingxSym(S.symbol);
...
(async () => {
  ...
  window.rfSelectExchange = function(ex) {
    ...
    // Passphrase: solo Bitget e Weex; WalletRow: solo Hyperliquid
  ...
  window.rfLoadApiKeysUI = function() {
    ...
    const isBybit = ex === 'bybit' || ex === 'bybit_demo';
    const isHL    = ex === 'hyperliquid';
    const walletRowL = document.getElementById('apiWalletRow');
  ...
  window.rfExportApiKeys = function() {
    ...
    exchanges.forEach(ex => {
      ...
      if (k) { data.keys[ex]={apiKey:k,secret:s||'',passphrase:p||''}; if(ex==='hyperliquid') data.keys[ex].walletAddress=localStorage.getItem('hl_wallet_address')||''; }
    });
