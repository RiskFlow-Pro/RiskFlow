...
const BYBIT_PROXY = 'https://bybit-proxy-cm8r.onrender.com';
...
const WEEX_PROXY = 'https://weex-proxy-qz51.onrender.com';
...
const BINGX_PROXY = 'https://bingx-proxy-g21v.onrender.com';
...
(async () => {
  ...
  const { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, sendPasswordResetEmail, onAuthStateChanged, signOut, GoogleAuthProvider, signInWithPopup } = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js');
  ...
  const firebaseConfig = {
    apiKey: "AIzaSyCQxU_EoIIFay4fDXma-wER2Mi3rme7Kls",
    authDomain: "riskflow-2ab5a.firebaseapp.com",
    projectId: "riskflow-2ab5a",
    storageBucket: "riskflow-2ab5a.firebasestorage.app",
    messagingSenderId: "341254065374",
    appId: "1:341254065374:web:d018072e909bcefae6c64f",
  };

  const fbApp = initializeApp(firebaseConfig);
  initializeAppCheck(fbApp, {
    provider: new ReCaptchaV3Provider('6Lc-EMssAAAAADiFLdPYY8T-HjNi7LxLRv3EC73o'),
  ...
  // ── BITGET PROXY ──
  const BITGET_PROXY = 'https://bitget-proxy-eu6s.onrender.com';
  ...
  async function syncKeysFromFirestore(uid) {
    ...
    if (savedEx && ['bitget','bybit','bybit_demo','weex','bingx'].includes(savedEx)) {
  ...
  onAuthStateChanged(auth, async user => {
    if (user) {
      currentUser = user;
      window._rfCurrentUid = user.uid; // expose for gdpr.js
      try {
        const snap = await getDoc(doc(db,'profiles',user.uid));
        const username = snap.exists() ? snap.data().username : user.email;
        await syncKeysFromFirestore(user.uid);
        rfShowApp(username);
        // Sync consenso cookie a Firestore se non ancora sincronizzato
        if (typeof rfSyncConsentToFirestore === 'function') {
          rfSyncConsentToFirestore(user.uid);
        }
  ...
  // ── GOOGLE LOGIN ──
  window.rfDoGoogleLogin = async function() {
    const errEl = document.getElementById('rf-login-err');
    try {
      const provider = new GoogleAuthProvider();
      const cred = await signInWithPopup(auth, provider);
      const uid = cred.user.uid;
      // Controlla se ha già un profilo, altrimenti crea con email come username
      const profileDoc = await getDoc(doc(db, 'profiles', uid));
      if (!profileDoc.exists()) {
        const baseUser = (cred.user.email||'user').split('@')[0].toLowerCase().replace(/[^a-z0-9_]/g,'').slice(0,24) || 'user_' + uid.slice(0,6);
        let username = baseUser;
        // Evita conflitti username
        const unameDoc = await getDoc(doc(db, 'usernames', username));
        if (unameDoc.exists()) username = baseUser + '_' + uid.slice(0,4);
        await setDoc(doc(db, 'usernames', username), { username, email: cred.user.email||'', uid });
        await setDoc(doc(db, 'profiles', uid), { username, createdAt: Date.now() });
      }
    } catch(e) {
      if (errEl) errEl.textContent = e.code === 'auth/popup-closed-by-user' ? '' : 'Errore Google login.';
    }
  };
  ...
  window.rfDoRegister = async function() {
    ...
    const terms = document.getElementById('rf-reg-terms') ? document.getElementById('rf-reg-terms').checked : true;
    const marketing = document.getElementById('rf-reg-marketing') ? document.getElementById('rf-reg-marketing').checked : false;
    const errEl = document.getElementById('rf-reg-err');
    const btn   = document.getElementById('rf-reg-btn');
    if (!terms) { errEl.textContent='Accetta i Termini di Servizio.'; return; }
    ...
    try {
      ...
      // Salva consensi GDPR su Firestore
      await setDoc(doc(db,'consents',cred.user.uid), {
        privacy: true,
        terms: true,
        marketing: marketing,
        analytics: false,
      ...
      // Salva anche in localStorage per banner cookie
      if (typeof rfGetConsent === 'undefined') {
        localStorage.setItem('rf_cookie_consent', JSON.stringify({
          version:'1.0', timestamp:new Date().toISOString(),
          necessary:true, analytics:false, marketing:marketing
        }));
      }
    } catch(e) {
    ...
    reader.onload = function(e) {
      try {
        ...
        exchanges.forEach(ex => {
          ...
          localStorage.setItem(lk.s, entry.secret || '');
