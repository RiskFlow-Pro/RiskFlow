...
// ── HYPERLIQUID ──
// Proxy per operazioni autenticate (firma EIP-712)
const HL_PROXY = 'https://hyperliquid-proxy.onrender.com';
// Endpoint pubblici (candele, ticker, pairs) 2014 diretto, no CORS issues
...
async function loadCandles(symbol,tf){
  ...
  const isHL    = window._activeExchange === 'hyperliquid';
  const data = isBingx ? await fetchBingxCandles(symbol,tf)
             ...
             : isHL    ? await fetchHLCandles(symbol,tf)
             : await fetchCandles(symbol,tf);
  document.getElementById('chartLoading').classList.add('hidden');
  const exchLabel = isBingx ? 'BingX' : isWeex ? 'Weex' : isBybit ? 'Bybit' : isHL ? 'Hyperliquid' : 'Bitget';
  if(data&&data.length){
...
function startTick(symbol){
  ...
  S.tickTimer=setInterval(async()=>{
    ...
    const t = isBingx ? await fetchBingxTicker(symbol)
            ...
            : isHL    ? await fetchHLTicker(symbol)
...
async function fetchContractInfo(symbol){
  ...
  if (isHL) {
    const info = await fetchHLContractInfo(symbol);
    _contractInfoCache[symbol] = info;
    return info;
  }
  try {
...
async function executeOrder(){
  ...
  if (isBingx) {
    ...
    try {
      // Set leverage prima dell'ordine
      try {
      ...
      const orderAction = {
        type:     'order',
        orders: [{
          a:         0,           // asset index — 0 usato come placeholder; il proxy risolve per coin
          b:         isBuy,       // isBuy
          p:         isMarket ? '0' : String(fmtPrice_(entry)),
          s:         contractStr, // size in contratti (coin)
          r:         false,       // reduceOnly
          t:         isMarket
            ? { market: {} }
            : { limit: { tif: 'Gtc' } },
          c:         'rf_' + Date.now(), // clientOrderId
        }],
        grouping: 'na',
      };
      // Risolvi asset index da meta
      try {
        const meta = await hlInfo({ type: 'meta' });
        const idx  = (meta?.universe || []).findIndex(a => a.name === coin);
        if (idx >= 0) orderAction.orders[0].a = idx;
      } catch(_) {}

      await window._hlRequest(orderAction);

      // SL separato (solo market, trigger order)
      if (sl && sl > 0) {
        try {
          const meta2  = await hlInfo({ type: 'meta' });
          ...
          await window._hlRequest(slAction);
        } catch(e) { console.warn('[HL SL] errore:', e.message); }
      }

      // TP (primo, se presente)
      if (tpList.length > 0) {
        try {
          const meta3  = await hlInfo({ type: 'meta' });
          const idx3   = (meta3?.universe || []).findIndex(a => a.name === coin);
          const tpAction = {
            type: 'order',
            orders: [{
              a:  idx3 >= 0 ? idx3 : 0,
              b:  !isBuy,
              p:  String(fmtPrice_(tpList[0])),
              s:  contractStr,
              r:  true,
          ...
          await window._hlRequest(tpAction);
        } catch(e) { console.warn('[HL TP] errore:', e.message); }
      }
      ...
      notify('✗ Errore ordine HL: ' + e.message, 'err');
...
async function executeLadderOrders(){
  ...
  try{
    if (isBingx) {
      ...
      try {
        const hlMeta0 = await hlInfo({ type: 'meta' });
        const hlIdx0  = (hlMeta0?.universe || []).findIndex(a => a.name === S.symbol.replace(/USDT$/i,''));
        await window._hlRequest({ type: 'updateLeverage', asset: hlIdx0 >= 0 ? hlIdx0 : 0, isCross: true, leverage: parseInt(lev) });
      } catch(e) { console.warn('HL set-leverage ladder:', e.message); }
    } else {
      await window._bitgetRequest('/api/v2/mix/account/set-margin-mode',{},{
        method:'POST',
        body:JSON.stringify({symbol:S.symbol,productType:'USDT-FUTURES',marginCoin:'USDT',marginMode:S.marginMode})
    ...
    for(let j=0;j<activeOrders.length;j++){
      ...
      if (isBingx) {
        ...
        const hlCoin = S.symbol.replace(/USDT$/i,'');
        const hlMeta = await hlInfo({ type: 'meta' });
        const hlIdx  = (hlMeta?.universe || []).findIndex(a => a.name === hlCoin);
        const hlAction = {
          type: 'order',
          orders: [{
            a:  hlIdx >= 0 ? hlIdx : 0,
            ...
            r:  false,
            t:  { limit: { tif: 'Gtc' } },
            c:  'rf_ladd_'+Date.now()+'_'+j,
          }],
          grouping: 'na',
        };
        await window._hlRequest(hlAction);
      } else {
...
(async () => {
  ...
  const db    = getFirestore(fbApp); // solo per auth (profiles/usernames)
  ...
  // ── LOCAL KEY HELPERS (localStorage only — no Firestore) ──
  // Le chiavi sono salvate in chiaro in localStorage, protette dal browser.
  ...
  async function saveKeysToFirestore(uid, apiKey, secret, passphrase, exchange) {
    ...
    const keys = _lsKeysFor(ex);
    localStorage.setItem(keys.k, apiKey);
    localStorage.setItem(keys.s, secret);
    if (keys.p && passphrase) localStorage.setItem(keys.p, passphrase);
    localStorage.setItem('rf_exchange', ex);
  }
  async function loadKeysFromFirestore(uid, exchange) {
    // Legge solo da localStorage
    const ex = exchange || _activeExchange || 'bitget';
    const keys = _lsKeysFor(ex);
    const apiKey = localStorage.getItem(keys.k) || '';
    const secret = localStorage.getItem(keys.s) || '';
    const passphrase = keys.p ? (localStorage.getItem(keys.p) || '') : '';
    if (!apiKey) return null;
    return { apiKey, secret, passphrase, exchange: ex };
  }
  async function deleteKeysFromFirestore(uid) {
    // No-op: la cancellazione viene gestita direttamente in rfDeleteApiKeys
  }
  async function syncKeysFromFirestore(uid) {
    // No-op: le chiavi sono già in localStorage, non serve sincronizzare
    // Ripristina solo l'exchange attivo se presente
    const savedEx = localStorage.getItem('rf_exchange');
    if (savedEx && ['bitget','bybit','bybit_demo','weex','bingx','hyperliquid'].includes(savedEx)) {
      _activeExchange = savedEx;
      window._activeExchange = savedEx;
    }
  }
  ...
  function loadActiveKeys() {
    ...
    if (ex === 'hyperliquid') return loadHyperliquidKeys();
    return loadBitgetKeys();
  }

  // (syncKeysFromFirestore è definita sopra insieme agli altri helper localStorage)
  ...
  // ── HYPERLIQUID REQUEST via Proxy (operazioni autenticate) ──
  // Il proxy firma con EIP-712 usando la private key dell'API Wallet
  async function hyperliquidRequest(action, _params={}, options={}) {
    ...
    const body  = JSON.stringify({ action, nonce, vaultAddress: null });
    const res   = await fetch(`${HL_PROXY}/exchange`, {
      method:  'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-hl-key':     privateKey,
        'x-hl-wallet':  walletAddr,
      },
      body,
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    if (data.status && data.status !== 'ok') throw new Error(data.response || 'Errore Hyperliquid');
    return data;
  }

  // ── HYPERLIQUID PUBLIC FETCH (info endpoint — no auth, no proxy) ──
  ...
  // Carica lista pairs Hyperliquid (perpetuals)
  async function loadHLPairs() {
    try {
      ...
      universe.forEach(asset => {
        ...
        if (!ASSETS.find(a => a.symbol === sym)) {
          ASSETS.push({ symbol: sym, _hlName: asset.name, cat: 'Crypto' });
        }
      });
    } catch(e) { console.warn('[HL] loadHLPairs:', e.message); }
  }

  // Candele Hyperliquid — endpoint: /info type=candleSnapshot
  async function fetchHLCandles(symbol, tf) {
    ...
    const startTime = endTime - interval * 60 * 1000 * 500; // ~500 candele
    try {
      const data = await hlInfo({
        type:      'candleSnapshot',
        req: { coin, interval: String(interval), startTime, endTime },
      });
      ...
      return data
        ...
        .filter(c => { if (seen.has(c.time)) return false; seen.add(c.time); return true; })
        .sort((a, b) => a.time - b.time);
    } catch(e) { console.warn('[HL] fetchHLCandles:', e.message); return null; }
  }

  // Ticker Hyperliquid (prezzo live)
  async function fetchHLTicker(symbol) {
    const coin = symbol.replace(/USDT$/i, '');
    try {
      ...
      return { last: price, open24h: price, high24h: price, low24h: price };
    } catch(e) { console.warn('[HL] fetchHLTicker:', e.message); return null; }
  }

  // Info contratto Hyperliquid (size step, decimali)
  async function fetchHLContractInfo(symbol) {
    const coin = symbol.replace(/USDT$/i, '');
    try {
      const data = await hlInfo({ type: 'meta' });
      const asset = (data?.universe || []).find(a => a.name === coin);
      if (!asset) return { pricePlace: 2, sizeMultiplier: 0.001, minTradeNum: 0.001 };
      const szDecimals  = asset.szDecimals  ?? 3;
      const sizeStep    = Math.pow(10, -szDecimals);
      return {
        pricePlace:      asset.pxDecimals ?? 2,
        sizeMultiplier:  sizeStep,
        minTradeNum:     sizeStep,
      };
    } catch(e) {
      return { pricePlace: 2, sizeMultiplier: 0.001, minTradeNum: 0.001 };
    }
  ...
  function activeRequest(endpoint, params={}, options={}) {
    ...
    if (ex === 'hyperliquid') return hyperliquidRequest(endpoint, params, options);
  ...
  window._hlRequest     = hyperliquidRequest;
    ...
    for (const p of _positions) {
      ...
      try {
        if (isBingx) {
          ...
          // HL close: piazza ordine market reduceOnly
          const coin    = p._hlCoin || p.symbol.replace(/USDT$/i,'');
          const szi     = p.total || 0;
          const isBuyClose = p.holdSide === 'short'; // chiude short con buy
          try {
            const meta = await hlInfo({ type: 'meta' });
            const idx  = (meta?.universe || []).findIndex(a => a.name === coin);
            const closeAction = {
              ...
              orders: [{
                ...
                r:  true,
                t:  { market: {} },
                c:  'rf_close_' + Date.now(),
              }],
              grouping: 'na',
            };
            await hyperliquidRequest(closeAction);
          } catch(e2) { throw e2; }
        } else {
    ...
    try {
      if (isBingx) {
        ...
        await hyperliquidRequest({
          type:  'cancel',
          cancels: [{ a: idx >= 0 ? idx : 0, o: parseInt(orderId) }],
        });
      } else {
  ...
  async function fetchBitgetDashboard() {
    ...
    const isBybit  = _activeExchange === 'bybit' || _activeExchange === 'bybit_demo';
    const isHL     = _activeExchange === 'hyperliquid';
    const exchLabel= isBingx ? 'BingX' : isWeex ? 'Weex' : isBybit ? (_activeExchange==='bybit_demo'?'Bybit Demo':'Bybit') : isHL ? 'Hyperliquid' : 'Bitget';
    ...
    try {
      ...
      if (isBingx) {
        ...
        // ── Hyperliquid balance ──
        const balData = await hlInfo({ type: 'clearinghouseState', user: loadHyperliquidKeys().secret });
        const marginSummary = balData?.marginSummary || {};
        equity    = parseFloat(marginSummary.accountValue        || 0);
        available = parseFloat(marginSummary.withdrawable        || marginSummary.accountValue || 0);
        upnl      = parseFloat(marginSummary.totalUnrealizedPnl  || 0);
        margin    = parseFloat(marginSummary.totalMarginUsed      || 0);

        // ── Hyperliquid positions ──
        const assetPositions = balData?.assetPositions || [];
  ...
  async function fetchPositionTPSLOrders(positions) {
    ...
    try {
      ...
      if (isHL) {
        // Hyperliquid: SL/TP embedded nelle posizioni (openOrders non distingue TP/SL facilmente)
        // Per ora mostriamo solo i dati embedded che arrivano da clearinghouseState
        positions.forEach((p, idx) => {
          const sym  = p.symbol || '';
          const side = (p.holdSide || 'long').toLowerCase();
          const key  = sym + '_' + side;
          map[key] = [];
          updatePosSLTPDisplay(idx, [], p);
        });
      } else if (isBingx) {
        ...
        try {
          ...
          const rawOrds = Array.isArray(ordData.data?.orders) ? ordData.data.orders : [];
          rawOrds.forEach(o => {
            const type = (o.type||'').toUpperCase();
            if (type !== 'STOP_MARKET' && type !== 'TAKE_PROFIT_MARKET') return;
            const sym      = (o.symbol||'').replace('-','');
            // positionSide: LONG o SHORT (hedge mode); in one-way può essere BOTH
            const posSide  = (o.positionSide||'').toLowerCase(); // 'long','short','both'
            const side     = posSide === 'both'
              ? ((o.side||'').toUpperCase() === 'SELL' ? 'long' : 'short')  // SELL chiude LONG
              : posSide;
            const key      = sym + '_' + side;
            ...
            if (!map[key]) map[key] = [];
            map[key].push({
              type:      type === 'STOP_MARKET' ? 'sl' : 'tp',
              price,
              orderId,
              planType:  type,
              size:      parseFloat(o.origQty||o.quantity||0),
            });
          });
        } catch(e) { console.warn('[TPSL BingX] fetch openOrders fallito:', e.message); }

        // Fallback: se per una posizione non abbiamo trovato ordini, proviamo i campi embedded
        positions.forEach((p, idx) => {
          const sym  = p.symbol || '';
          const side = (p.holdSide||'long').toLowerCase();
          const key  = sym+'_'+side;
          if (!map[key] || map[key].length === 0) {
            const list = [];
            const slPx = parseFloat(p.stopLoss||p.stopLossPrice||0);
            const tpPx = parseFloat(p.takeProfit||p.takeProfitPrice||0);
  ...
  window.cancelTpslOrder = async function(symbol, orderId, planType, label) {
    ...
    try {
      ...
      } else if (!isBybit && !isWeex && !isBingx && !isHL) {
  ...
  function rfShowApp(username) {
    ...
    if(!_appInitDone){ _appInitDone=true;
      ...
      const isBybit = _activeExchange === 'bybit' || _activeExchange === 'bybit_demo';
      const isHL    = _activeExchange === 'hyperliquid';
      (isBingx ? loadBingxPairs() : isWeex ? loadWeexPairs() : isBybit ? loadBybitPairs() : isHL ? loadHLPairs() : loadBitgetPairs()).then(()=>loadCandles(S.symbol, S.tf));
  ...
  window.rfSelectExchange = function(ex) {
    ...
    localStorage.setItem('rf_exchange', ex);
    ['bitget','bybit','bybit_demo','weex','bingx','hyperliquid'].forEach(e => {
      const idMap = {bitget:'exBtnBitget', bybit:'exBtnBybit', bybit_demo:'exBtnBybitDemo', weex:'exBtnWeex', bingx:'exBtnBingx', hyperliquid:'exBtnHyperliquid'};
      const btn = document.getElementById(idMap[e]);
      if(btn) btn.classList.toggle('active', e === ex);
    });
    // Passphrase: solo Bitget e Weex; Wallet address: solo Hyperliquid
    ...
    if(secretInput) secretInput.placeholder = ex === 'hyperliquid' ? 'non richiesto — usa wallet address sotto' : 'secret key';
    ...
    if (_appInitDone) {
      ...
      const isHLEx    = ex === 'hyperliquid';
      (isBingxEx ? loadBingxPairs() : isWeexEx ? loadWeexPairs() : isBybitEx ? loadBybitPairs() : isHLEx ? loadHLPairs() : loadBitgetPairs()).then(() => loadCandles(S.symbol, S.tf));
    }
  };
  ...
  window.rfLoadApiKeysUI = function() {
    ...
    const isBybit = ex === 'bybit' || ex === 'bybit_demo';
    const isHL    = ex === 'hyperliquid';
    // Show/hide wallet row
    const walletRow = document.getElementById('apiWalletRow');
    if (walletRow) walletRow.style.display = isHL ? '' : 'none';
    const passRow2 = document.getElementById('apiPassRow');
    if (passRow2) passRow2.style.display = (ex === 'bitget' || ex === 'weex') ? '' : 'none';
    const lsKey = _lsKeysFor(ex).k;
    const localKey = localStorage.getItem(lsKey);
    if (localKey) {
      document.getElementById('apiKeyInput').value = '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022';
      if (!isHL) document.getElementById('apiSecretInput').value = '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022';
      if (isHL) {
        const hlWallet = localStorage.getItem('hl_wallet_address');
        const walletEl = document.getElementById('apiWalletInput');
        if (walletEl) walletEl.value = hlWallet ? hlWallet.slice(0,6)+'...'+hlWallet.slice(-4) : '';
      }
      if (isWeex) document.getElementById('apiPassInput').value = localStorage.getItem('weex_api_passphrase') ? '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022' : '';
      else if (!isBybit && !isBingx && !isHL) document.getElementById('apiPassInput').value = localStorage.getItem('bitget_api_passphrase') ? '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022' : '';
  ...
  window.rfSaveApiKeys = function() {
    ...
    if (isHL && !walletVal) { if(statusEl){statusEl.textContent='Wallet Address obbligatorio per Hyperliquid';statusEl.className='api-status err';} return; }

    const lsKeys = _lsKeysFor(ex);
    const lsKey    = lsKeys.k;
    const lsSecret = lsKeys.s;
    const lsPass   = lsKeys.p;
  ...
  window.rfDeleteApiKeys = function() {
    ...
    document.getElementById('apiSecretInput').value='';
    document.getElementById('apiPassInput').value='';
    const walletEl2 = document.getElementById('apiWalletInput'); if(walletEl2) walletEl2.value='';
    const statusEl = document.getElementById('apiStatus');
    if (statusEl) { statusEl.textContent='Nessuna API key salvata per ' + ex; statusEl.className='api-status err'; }
    document.getElementById('apiModal').classList.remove('open');
    document.getElementById('accBalance').textContent='—';
  ...
  window.rfExportApiKeys = function() {
    const exchanges = ['bitget', 'bybit', 'bybit_demo', 'weex', 'bingx', 'hyperliquid'];
    const data = { version: 1, exportedAt: new Date().toISOString(), keys: {} };
    exchanges.forEach(ex => {
      const lk = _lsKeysFor(ex);
      const k = localStorage.getItem(lk.k);
      const s = localStorage.getItem(lk.s);
      const p = lk.p ? localStorage.getItem(lk.p) : null;
      if (k) {
        data.keys[ex] = { apiKey: k, secret: s || '', passphrase: p || '' };
        if (ex === 'hyperliquid') data.keys[ex].walletAddress = localStorage.getItem('hl_wallet_address') || '';
      }
    });
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'riskflow-api-keys.json';
    a.click();
  ...
  window.rfImportApiKeys = function(input) {
    ...
    reader.onload = function(e) {
      try {
        ...
        let count = 0;
        exchanges.forEach(ex => {
          const entry = data.keys[ex];
          if (!entry || !entry.apiKey) return;
          const lk = _lsKeysFor(ex);
          localStorage.setItem(lk.k, entry.apiKey);
          if (ex !== 'hyperliquid') localStorage.setItem(lk.s, entry.secret || '');
          if (ex === 'hyperliquid' && entry.walletAddress) localStorage.setItem('hl_wallet_address', entry.walletAddress);
        ...
        if (data.activeExchange && exchanges.includes(data.activeExchange)) {
          ...
          _activeExchange = data.activeExchange;
          window._activeExchange = data.activeExchange;
        }
        notify(count + ' exchange importati ✓', 'ok');
        if (window.rfLoadApiKeysUI) window.rfLoadApiKeysUI();
      } catch(err) {
        notify('Errore importazione: ' + err.message, 'err');
      }
